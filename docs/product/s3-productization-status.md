...
协议存在不代表运行时存在。PC 原型和互操作安全 fixture 完成前，产品必须保持
`DEFERRED_PC_RUNTIME_MISSING`。

## 发布边界

ASR-005 仍为 `BLOCKED` / `SKIPPED_PENDING_USER_TEST`；人工执行延后不等于
通过。完整 S3、发布高端兼容性、EXP-005 正式包和发布交付也仍未闭合，所以
preflight 必须继续返回 `NOT RELEASE-READY`。
