...
| 时间戳转写片段 | 结构化能力与生产分段契约已实现，独立准确率门禁阻塞；人工执行暂时跳过 | S2 Mobile Core | 生产 ASR 返回并事务持久化有序 start/end 片段；2026-07-25 Xiaomi 固定 clip 预测为 5/4 段，证据 SHA-256=`01b77e52dd6eadd048a6a2cd91952e7502ad9f6e856010b08d32923a4be1c0d7`；provisional 工程 P95=182 ms/18 边界 | `ASR-005-TIMESTAMP-INDEPENDENT` 仍 BLOCKED，执行状态为 `SKIPPED_PENDING_USER_TEST`：参考为 provisional 且无独立 reviewer，`releaseEligible=false`。等待用户后续测试；它是唯一未通过的 Mobile Core 功能门禁 |
...
| 端侧匿名说话人分离 | `DEFERRED_NO_ADMISSIBLE_CANDIDATE` | S3 | Sherpa AAR v1.13.3、fixture 和阈值保持固定；FP32 有界候选与唯一 INT8 fallback 的 Xiaomi 5 分钟覆盖率/DER 分别为 92.235%/13.689% 与 93.645%/12.192%，但语义门禁均失败 | projected RTF 分别为 2.3348 与 2.4020，超过 0.5；不运行 120 分钟、不推进第三候选、不打包模型、不提供入口、不保存 voiceprint；30 分钟 `SKIPPED_BY_PLAN` |
| PC 配对 AI provider | `DEFERRED_PC_RUNTIME_MISSING` | S3 后续 | provider v1 schema 固定短时 QR、公钥指纹、一次性 challenge、能力协商、job/idempotency/input hash、进度/取消/重试、严格结果和 replay 拒绝 | 仅协议，不存在 PC runtime、移动 adapter、相机权限或扫码入口；协议存在不能标记产品 PASS |
