...
本节用于同步后续跨端产品语言，不纳入本移动端首轮画板：

- 桌面端保留自定义云端模型。列表以本地图标、唯一模型 ID、可选配置名称和独立选择状态呈现；当前模型需先切换后才能编辑或删除。删除入口仅放在编辑弹窗左侧，内部 profile ID 与密钥引用不展示。
- 自定义模型使用用户自己的 API key 并由桌面端直连；密钥保存在 macOS Keychain，但客户端提示词不承诺保密。
- 未完成或可重试任务仍引用当前密钥时，不允许更换密钥或删除模型；确认后的执行由一次性准备凭证绑定，配置变化后必须重新确认。
- 后续内置平台模型作为同一列表中的锁定项，不允许编辑或删除，也不取代用户自定义模型；当前阶段不展示未启用的内置项。
- 内置模型依赖账号认证、会员权益、配额、计费和服务端代理，由平台保管供应商密钥与受保护提示词，属于下一阶段规划。
- 桌面设置不展示 FileVault 或独立“隐私与安全”模块；后端仍保留设备安全状态探测，且不能把它与 Keychain 密钥保护混为一谈。
