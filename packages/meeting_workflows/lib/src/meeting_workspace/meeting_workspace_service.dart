...
import 'meeting_workspace_port.dart';

class MeetingWorkspaceService {
  const MeetingWorkspaceService({required MeetingWorkspacePort port})
    : _port = port;

  final MeetingWorkspacePort _port;
  ...
  Future<List<MeetingWorkspaceSummary>> listMeetings({
    ...
    if (limit < 1 || limit > 500 || offset < 0) {
      throw ArgumentError('meeting library window is invalid');
    }
    return _port.listMeetings(
      query: query.trim(),
      limit: limit,
      offset: offset,
    );
  ...
  Future<MeetingWorkspaceSnapshot?> openMeeting(int recordingId) {
    ...
    return _port.openMeeting(recordingId);
  }
  ...
  Future<bool> saveSegment({
    ...
    MeetingWorkspaceReviewState reviewState =
  ...
  Future<void> assignSpeaker({
    ...
    if (generationId <= 0 ||
        segmentId <= 0 ||
        state == MeetingWorkspaceSpeakerState.assigned && speakerId == null ||
        state != MeetingWorkspaceSpeakerState.assigned && speakerId != null) {
  ...
  MeetingWorkspaceExport export(
    MeetingWorkspaceSnapshot workspace,
    MeetingWorkspaceExportFormat format,
  ) {
    return switch (format) {
      MeetingWorkspaceExportFormat.text => MeetingWorkspaceExport(
        format: format,
        fileExtension: 'txt',
        mimeType: 'text/plain',
        contents: _plainText(workspace),
      ),
      MeetingWorkspaceExportFormat.markdown => MeetingWorkspaceExport(
        format: format,
        fileExtension: 'md',
        mimeType: 'text/markdown',
        contents: _markdown(workspace),
      ),
      MeetingWorkspaceExportFormat.webVtt => MeetingWorkspaceExport(
        format: format,
        fileExtension: 'vtt',
        mimeType: 'text/vtt',
        contents: _timed(workspace, webVtt: true),
      ),
      MeetingWorkspaceExportFormat.srt => MeetingWorkspaceExport(
        format: format,
        fileExtension: 'srt',
        mimeType: 'application/x-subrip',
        contents: _timed(workspace, webVtt: false),
      ),
      MeetingWorkspaceExportFormat.json => MeetingWorkspaceExport(
        format: format,
        fileExtension: 'json',
  ...
  String _plainText(MeetingWorkspaceSnapshot workspace) => workspace.segments
      .map((segment) => '${_speaker(segment)}：${segment.text}')
      .join('\n');

  String _markdown(MeetingWorkspaceSnapshot workspace) {
  ...
  String _timed(MeetingWorkspaceSnapshot workspace, {required bool webVtt}) {
  ...
  Map<String, Object?> _json(MeetingWorkspaceSnapshot workspace) => {
    'schemaVersion': 1,
    'recording': {
  ...
  String _speaker(MeetingWorkspaceSegment segment) =>
