...
void main() {
  test('five formats have exact full and range-aware output', () async {
    ...
    final directory = await Directory.systemTemp.createTemp('meeting-export-');
    ...
    final service = MeetingExportService(
    ...
    final full = <MeetingExportFormat, String>{};
    for (final format in MeetingExportFormat.values) {
      final receipt = await service.export(
        ...
        segments: segments,
        format: format,
        selection: const MeetingExportSelection.all(),
      );
    ...
    expect(
      full[MeetingExportFormat.markdown],
      '# 项目 周会\n'
      '\n'
      '- **00:00–00:01** 你好  \n'
      '  <world>\n'
    ...
    expect(jsonDecode(full[MeetingExportFormat.json]!), <String, Object?>{
      'title': '项目 周会',
    ...
    final fullVtt = full[MeetingExportFormat.vtt]!;
    expect(
      fullVtt,
      'WEBVTT\n'
      '\n'
    ...
    final selection = MeetingExportSelection.range(
      MeetingTimeRange(startMs: 1000, endMs: 6000, durationMs: 4000000),
    );
    final ranged = <MeetingExportFormat, String>{};
    for (final format in MeetingExportFormat.values) {
      final receipt = await service.export(
        recordingId: recordingId,
        title: '项目 周会',
    ...
    expect(ranged[MeetingExportFormat.text], isNot(contains('over hour')));
    expect(ranged[MeetingExportFormat.markdown], isNot(contains('over hour')));
    final rangedJson =
        jsonDecode(ranged[MeetingExportFormat.json]!) as Map<String, Object?>;
    ...
    expect(ranged[MeetingExportFormat.srt], startsWith('1\n'));
    expect(ranged[MeetingExportFormat.srt], contains('\n3\n'));
    expect(ranged[MeetingExportFormat.srt], isNot(contains('\n4\n')));
    expect(
      ranged[MeetingExportFormat.srt],
      contains('00:00:00,000 --> 00:00:01,234'),
    );
    _expectValidVtt(ranged[MeetingExportFormat.vtt]!, expectedCueCount: 3);
  ...
  test('empty range and sink failure leave no asset or partial file', () async {
    final fixture = await openRecordingTestDatabase();
    addTearDown(fixture.database.close);
    final directory = await Directory.systemTemp.createTemp('meeting-fail-');
    addTearDown(() => directory.delete(recursive: true));
    ...
    await expectLater(
      service.export(
        ...
        selection: MeetingExportSelection.range(
          MeetingTimeRange(startMs: 2000, endMs: 3000, durationMs: 10000),
        ),
      ),
      throwsStateError,
    );

    final failing = MeetingExportService(
      recordingsRepository: repository,
      exportDirectory: directory,
  ...
  test('large export writes a complete ordered file', () async {
    final fixture = await openRecordingTestDatabase();
    addTearDown(fixture.database.close);
    final directory = await Directory.systemTemp.createTemp('meeting-large-');
    addTearDown(() => directory.delete(recursive: true));
    final recordingId = await fixture.database.insert(
      'recordings',
      <String, Object?>{
  ...
  test('failed replacement restores the previous successful export', () async {
    final fixture = await openRecordingTestDatabase();
    addTearDown(fixture.database.close);
    final directory = await Directory.systemTemp.createTemp('meeting-replace-');
    addTearDown(() => directory.delete(recursive: true));
    final recordingId = await fixture.database.insert(
      'recordings',
      <String, Object?>{
    ...
    final repository = RecordingsRepository(database: fixture.appDatabase);
    final successful = MeetingExportService(
      recordingsRepository: repository,
      exportDirectory: directory,
    );
