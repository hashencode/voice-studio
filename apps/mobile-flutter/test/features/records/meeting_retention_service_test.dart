...
import 'package:voice2text_flutter/features/records/service/meeting_retention_service.dart';
...
void main() {
  ...
  test('default disabled policy never deletes recently deleted data', () async {
    ...
    expect(result.status, MeetingRetentionScanStatus.disabled);
  ...
  for (final retentionDays in AppSettings.supportedRetentionDays) {
    test(
      ...
      () async {
        ...
        expect(result.status, MeetingRetentionScanStatus.completed);
  ...
  test('file failure stays pending and the next scan retries it', () async {
    ...
    expect(first.status, MeetingRetentionScanStatus.partial);
    ...
    expect(second.status, MeetingRetentionScanStatus.completed);
...
MeetingRetentionService _service({
  ...
  return MeetingRetentionService(
...
Future<int> _insertRecording(
  ...
  final commit = await repository.insertImported(
    filePath: '/data/user/0/app/files/meetings/imports/$fingerprint/audio.m4a',
...
class _AllowFileStore implements MeetingFileStore {
  @override
