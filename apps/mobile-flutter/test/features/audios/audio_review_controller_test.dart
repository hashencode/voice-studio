...
void main() {
  test('current segment lookup uses start-inclusive end-exclusive bounds', () {
    ...
    expect(AudioReviewController.indexForPosition(segments, 0), 0);
    expect(AudioReviewController.indexForPosition(segments, 999), 0);
    expect(AudioReviewController.indexForPosition(segments, 1000), 1);
    expect(AudioReviewController.indexForPosition(segments, 2000), isNull);
    expect(AudioReviewController.indexForPosition(segments, 2500), 2);
    expect(AudioReviewController.indexForPosition(segments, 3000), isNull);
  });
  ...
  test('lookup remains correct for thousands of segments', () {
    ...
    expect(AudioReviewController.indexForPosition(segments, 4321456), 4321);
  });
  ...
  test(
    ...
    () async {
      ...
      AudioReviewController buildController() => AudioReviewController(
        ...
        playbackService: AudioPlaybackService(backend: _FakePlaybackBackend()),
      );

      final controller = buildController();
      addTearDown(controller.dispose);
      await controller.load();
      final segmentId = controller.audio!.segments.single.id;
      expect(controller.audio!.segments.single.confidence, 0.42);
      expect(controller.canUndo, isFalse);
      expect(controller.canRedo, isFalse);

      await controller.saveSegment(segmentId: segmentId, text: '修订');
      expect(controller.audio!.segments.single.text, '修订');
      expect(controller.audio!.segments.single.confidence, 0.42);
      expect(controller.canUndo, isTrue);
      expect(controller.canRedo, isFalse);

      expect(await controller.undoLastEdit(), isTrue);
      expect(controller.audio!.segments.single.text, '原文');
      expect(controller.canUndo, isFalse);
      ...
      expect(reopened.audio!.segments.single.text, '修订');
      ...
      expect(
        reopened.audio!.segments.single.reviewState,
        TranscriptReviewState.needsReview,
      );
      expect(reopened.audio!.segments.single.confidence, 0.42);
      expect(
        await reopened.updateReviewState(
          segmentId: segmentId,
      ...
      expect(reopened.audio!.segments.single.confidence, 0.42);
      expect(reopened.audio!.segments.single.reviewedAtMs, isNotNull);
      expect(
        await reopened.updateReviewState(
          segmentId: segmentId,
      ...
      expect(reopened.audio!.segments.single.reviewedAtMs, isNull);
  ...
  test('new query and clear suppress stale search results', () async {
    ...
    final controller = AudioReviewController(
      recordingId: 1,
      searchService: searchService,
      playbackService: AudioPlaybackService(backend: backend),
    );
    addTearDown(controller.dispose);
    const oldQuery = AudioTranscriptQuery(text: 'old');
    const newQuery = AudioTranscriptQuery(text: 'new');
  ...
  test('database load failure exits the loading state', () async {
    ...
    expect(controller.error, '音频工作区加载失败');
  });
  ...
  test(
    ...
    () async {
      ...
      expect(controller.audio!.annotations, hasLength(1));
      expect(
        controller.audio!.annotations.single.kind,
        RecordingAnnotationKind.marker,
      );
      await controller.seekToAnnotation(controller.audio!.annotations.single);
...
class _ControlledSearchService extends AudioSearchService {
  ...
  Future<List<TranscriptSegmentEntity>> search({
    ...
    required AudioTranscriptQuery query,
    int limit = 200,
  }) {
