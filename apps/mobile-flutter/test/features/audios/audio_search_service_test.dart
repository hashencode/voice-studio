...
void main() {
  test(
    ...
    () async {
      ...
      final service = AudioSearchService(database: fixture.appDatabase);

      expect(
        await service.search(
          recordingId: seeded.recordingId,
          query: const AudioTranscriptQuery(),
        ),
        isEmpty,
      );
      expect(
        (await service.search(
          recordingId: seeded.recordingId,
          query: const AudioTranscriptQuery(text: 'ALPHA'),
        )).map((item) => item.text),
      ...
      expect(
        (await service.search(
          recordingId: seeded.recordingId,
          query: const AudioTranscriptQuery(text: '你好'),
        )).map((item) => item.text),
      ...
      expect(
        (await service.search(
          recordingId: seeded.recordingId,
          query: const AudioTranscriptQuery(text: '%'),
        )).map((item) => item.text),
        const <String>['Alpha 你好 100%'],
      );
      expect(
        (await service.search(
          recordingId: seeded.recordingId,
          query: const AudioTranscriptQuery(text: '_'),
        )).map((item) => item.text),
      ...
      expect(
        await service.search(
          recordingId: seeded.recordingId,
          query: const AudioTranscriptQuery(text: 'missing'),
        ),
        isEmpty,
      );
      expect(
        await service.search(
          ...
          query: const AudioTranscriptQuery(text: 'ghost'),
        ),
        isEmpty,
      );
    },
