...
void main() {
  testWidgets('home import panel completes and reloads the shared repository', (
    ...
    expect(find.text('导入会议媒体'), findsOneWidget);
    ...
    expect(find.text('导入会议媒体'), findsNothing);
...
class _SuccessfulImportService extends MeetingImportService {
  int pickCalls = 0;

  @override
  Future<MeetingImportOutcome?> pickAndImport() async {
    pickCalls += 1;
    return const MeetingImportOutcome(
      candidate: ImportCandidate(
        path:
            '/data/user/0/app/files/meetings/imports/complete/integration.m4a',
...
class _SuccessfulSharedImportService extends MeetingImportService {
  ...
  Future<MeetingImportOutcome?> consumeSharedImport() async {
    ...
    return const MeetingImportOutcome(
      candidate: ImportCandidate(
        path: '/data/user/0/app/files/meetings/imports/complete/shared.m4a',
...
class _OutOfOrderRecordingsRepository extends RecordingsRepository {
  ...
  Future<List<RecordingEntity>> listActive({String? groupName}) {
    ...
    return Future<List<RecordingEntity>>.value(<RecordingEntity>[
      RecordingEntity(
        id: 8,
        filePath: '/data/user/0/app/files/meetings/imports/complete/shared.m4a',
        displayName: '系统分享.m4a',
        groupName: null,
        deletedAtMs: null,
        isFavorite: false,
