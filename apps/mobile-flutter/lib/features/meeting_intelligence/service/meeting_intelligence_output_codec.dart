...
class MeetingIntelligenceOutputCodec {
  const MeetingIntelligenceOutputCodec();

  static const schemaVersion = 'meeting_intelligence_output/v1';
  ...
  MeetingIntelligenceOutput decode(String source) {
    ...
    if (version != schemaVersion) {
      throw FormatException('不支持的会议智能输出版本：$version');
    }
    final rawItems = root['items'];
    if (rawItems is! List || rawItems.length > 200) {
      throw const FormatException('会议智能条目列表无效');
    }
    final items = <MeetingInsightCandidate>[];
    ...
    return MeetingIntelligenceOutput(
      ...
      meetingType: _optionalString(root, 'meeting_type', maximum: 100),
  ...
  MeetingInsightCandidate _decodeItem(Object? value, int index) {
    ...
    final evidence = <MeetingEvidenceCandidate>[];
    for (
      var evidenceIndex = 0;
    ...
    final resolutionName =
        ...
        MeetingInsightResolutionState.notApplicable.name;
    final resolutionState = _findByName(
      MeetingInsightResolutionState.values,
      (candidate) => candidate.name,
      resolutionName,
    );
    ...
    return MeetingInsightCandidate(
      kind: kind,
