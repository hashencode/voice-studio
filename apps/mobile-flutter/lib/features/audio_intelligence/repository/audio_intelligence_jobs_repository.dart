...
class AudioIntelligenceJobsRepository {
  AudioIntelligenceJobsRepository({AppDatabase? database})
  ...
  Future<AudioIntelligenceJobEntity> createOrGet({
    required AudioIntelligenceProvider provider,
    required AudioIntelligenceRequest request,
    ...
    if (request.consentDecision != AudioConsentDecision.granted ||
      ...
      throw StateError('创建云端任务前必须获得本次音频同意');
    }
    final dedupeKey = buildDedupeKey(provider: provider, request: request);
    final db = await _database.database;
    return db.transaction<AudioIntelligenceJobEntity>((transaction) async {
      final existing = await _findByDedupeKey(transaction, dedupeKey);
      if (existing != null) return existing;
      final now = DateTime.now().millisecondsSinceEpoch;
      await transaction.insert('audio_intelligence_jobs', <String, Object?>{
        ...
        'processing_location': request.processingLocation.name,
        'template_id': request.templateId.name,
        'status': AudioIntelligenceJobStatus.queued.name,
        'progress': 0.0,
        'attempt_count': 0,
        'cancel_requested': 0,
      ...
      if (created == null) throw StateError('无法创建音频智能任务');
      return created;
    });
  }

  Future<AudioIntelligenceJobEntity?> findById(int id) async {
    final db = await _database.database;
    ...
    return rows.isEmpty
        ? null
        : AudioIntelligenceJobEntity.fromMap(rows.single);
  }
  ...
  Future<List<AudioIntelligenceJobEntity>> listUnfinished() async {
    final db = await _database.database;
    final rows = await db.query(
      'audio_intelligence_jobs',
      where: 'status IN (?, ?)',
      whereArgs: <Object>[
        AudioIntelligenceJobStatus.queued.name,
        AudioIntelligenceJobStatus.processing.name,
      ],
      orderBy: 'created_at_ms ASC, id ASC',
    );
    return rows.map(AudioIntelligenceJobEntity.fromMap).toList(growable: false);
  }

  Future<AudioIntelligenceJobEntity> markProcessing(int id) async {
    final db = await _database.database;
    final now = DateTime.now().millisecondsSinceEpoch;
    final updated = await db.rawUpdate(
      '''
      UPDATE audio_intelligence_jobs
      SET status = ?,
    ...
    if (updated != 1) throw StateError('音频智能任务状态已变化');
  ...
  Future<void> updateProgress(int id, double progress) async {
    ...
    await db.update(
      ...
      where: 'id = ? AND status = ?',
      whereArgs: <Object>[id, AudioIntelligenceJobStatus.processing.name],
    );
  }
  ...
  Future<void> markCompleted(int id) {
    ...
    return _updateExpected(
      id: id,
      expected: const <AudioIntelligenceJobStatus>{
        AudioIntelligenceJobStatus.processing,
      },
      values: <String, Object?>{
        'status': AudioIntelligenceJobStatus.completed.name,
        'progress': 1.0,
        'completed_at_ms': now,
        'heartbeat_at_ms': now,
        'updated_at_ms': now,
  ...
  Future<void> markFailed(int id, String errorCode) {
    ...
    return _updateExpected(
      ...
      values: <String, Object?>{
        'status': AudioIntelligenceJobStatus.failed.name,
  ...
  Future<void> requestCancel(int id) async {
    ...
    await db.transaction<void>((transaction) async {
      ...
      if (rows.isEmpty) throw StateError('音频智能任务不存在');
      final status = AudioIntelligenceJobStatus.fromStorage(
        rows.single['status'],
      );
      final now = DateTime.now().millisecondsSinceEpoch;
      if (status == AudioIntelligenceJobStatus.queued) {
        await transaction.update(
          'audio_intelligence_jobs',
          <String, Object?>{
            'status': AudioIntelligenceJobStatus.canceled.name,
            'cancel_requested': 1,
            'completed_at_ms': now,
            'updated_at_ms': now,
          },
      ...
      } else if (status == AudioIntelligenceJobStatus.processing) {
        await transaction.update(
          'audio_intelligence_jobs',
  ...
  Future<void> requeueForUserRetry(int id) async {
    ...
    await _updateExpected(
      id: id,
      expected: const <AudioIntelligenceJobStatus>{
        AudioIntelligenceJobStatus.failed,
  ...
  Future<void> reconcileQueued(int id) async {
    ...
    await db.update(
      ...
      where: 'id = ? AND status = ?',
      whereArgs: <Object>[id, AudioIntelligenceJobStatus.queued.name],
    );
  }
  ...
  Future<void> markRecoveryUnknown(int id) async {
    ...
    await _updateExpected(
      ...
      values: <String, Object?>{
        'status': AudioIntelligenceJobStatus.recoveryUnknown.name,
  ...
  Future<AudioIntelligenceJobEntity?> _findByDedupeKey(
  ...
  Future<void> _updateExpected({
    required int id,
    required Set<AudioIntelligenceJobStatus> expected,
