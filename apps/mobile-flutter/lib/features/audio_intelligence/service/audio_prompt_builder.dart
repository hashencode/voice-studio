...
class AudioPrompt {
  const AudioPrompt({required this.system, required this.user});
...
class AudioPromptBuilder {
  const AudioPromptBuilder({this.codec = const AudioIntelligenceOutputCodec()});

  final AudioIntelligenceOutputCodec codec;

  AudioPrompt build(AudioIntelligenceRequest request) {
    final template = AudioTemplate.find(request.templateId);
    ...
    return AudioPrompt(
      system: '''
你是音频纪要结构化提取器。只返回一个 JSON 对象，不要 Markdown。
输出必须严格符合 audio_intelligence_output/v1 和提供的示例字段。
