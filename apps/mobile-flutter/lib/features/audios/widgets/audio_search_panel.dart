...
import '../service/audio_search_service.dart';
import '../utils/audio_time_text.dart';

class AudioSearchPanel extends StatefulWidget {
  ...
  final ValueChanged<AudioTranscriptQuery> onSearch;
  final VoidCallback onClear;
  final ValueChanged<TranscriptSegmentEntity> onSelect;
  final bool searching;

  @override
  State<AudioSearchPanel> createState() => _AudioSearchPanelState();
}

class _AudioSearchPanelState extends State<AudioSearchPanel> {
  ...
  AudioTranscriptQuery _currentQuery = const AudioTranscriptQuery();
          ...
          Wrap(
            ...
            children: <Widget>[
              ...
              if (_currentQuery.timeRange case final range?)
                GooTag(
                  label:
                      '时间 ${formatAudioClock(range.startMs)}–'
                      '${formatAudioClock(range.endMs)}',
                  style: GooTagStyle.assist,
  ...
  void _handleSearchClear() {
    if (_reviewFilter == _ReviewFilter.all && !_timeTouched) {
      setState(() => _currentQuery = const AudioTranscriptQuery());
      widget.onClear();
      return;
    }
    _emitQuery();
  ...
  void _emitQuery() {
    ...
    final range = _timeTouched && (startMs != 0 || endMs != widget.durationMs)
        ...
        : null;
    final query = AudioTranscriptQuery(
  ...
  String? _validateRange(int? startMs, int? endMs) {
    ...
    if (endMs > widget.durationMs) return '结束时间不能超过音频时长';
    return null;
  }

  void _clearAll() {
    _searchController.clear();
    _startController.text = formatAudioClock(0);
    _endController.text = formatAudioClock(widget.durationMs);
    setState(() {
      _reviewFilter = _ReviewFilter.all;
      _timeTouched = false;
      _rangeError = null;
      _currentQuery = const AudioTranscriptQuery();
    });
    widget.onClear();
  }
