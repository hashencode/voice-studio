...
import '../service/audio_playback_service.dart';
import '../service/audio_search_service.dart';

class AudioReviewController extends ChangeNotifier {
  AudioReviewController({
    ...
    AudioPlaybackService? playbackService,
       ...
       _searchService = searchService ?? AudioSearchService(),
       playback = playbackService ?? AudioPlaybackService();
  ...
  final AudioSearchService _searchService;
  ...
  AudioTranscriptQuery _searchQuery = const AudioTranscriptQuery();
  ...
  AudioTranscriptQuery get searchQuery => _searchQuery;
  ...
  Future<bool> undoLastEdit() async {
    final generation = _audio?.generation;
  ...
  Future<bool> updateReviewState({
    ...
    final activeGenerationId = _audio?.generation?.id;
  ...
  Future<void> search(AudioTranscriptQuery query) async {
    if (query.isEmpty) {
      clearSearch();
      return;
    }
  ...
  void clearSearch() {
    _searchEpoch += 1;
    _searchQuery = const AudioTranscriptQuery();
    _searchResults = const <TranscriptSegmentEntity>[];
    _searching = false;
    notifyListeners();
  }
  ...
  void _updateCurrentSegment() {
    _currentSegmentIndex = indexForPosition(
      _audio?.segments ?? const <TranscriptSegmentEntity>[],
