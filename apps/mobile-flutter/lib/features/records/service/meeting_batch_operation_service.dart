...
enum MeetingBatchItemStatus { succeeded, skipped, failed }

class MeetingBatchItemResult {
  const MeetingBatchItemResult({
  ...
  final MeetingBatchItemStatus status;
  final String reason;
  final String? entryName;
}

class MeetingBatchOperationResult {
  const MeetingBatchOperationResult({required this.items});

  final List<MeetingBatchItemResult> items;

  MeetingBatchItemResult forRecording(int recordingId) {
    return items.singleWhere((item) => item.recordingId == recordingId);
  }

  int get succeededCount => items
      .where((item) => item.status == MeetingBatchItemStatus.succeeded)
      .length;

  int get skippedCount => items
      .where((item) => item.status == MeetingBatchItemStatus.skipped)
      .length;

  int get failedCount => items
      .where((item) => item.status == MeetingBatchItemStatus.failed)
      .length;
}

class MeetingBatchExportResult extends MeetingBatchOperationResult {
  const MeetingBatchExportResult({
...
class MeetingBatchOperationService {
  MeetingBatchOperationService({
    ...
    RetryRecordings? retryRecordings,
       ...
       _meetingExportService =
           meetingExportService ??
           MeetingExportService(recordingsRepository: recordingsRepository),
  ...
  final MeetingDeletionCoordinator _meetingDeletionCoordinator;
  final MeetingExportService _meetingExportService;
  final EphemeralShareArtifactService _ephemeralShareArtifactService;
  final RetryRecordings? _retryRecordings;

  Future<MeetingBatchOperationResult> move(
    Iterable<int> recordingIds, {
    ...
    final results = <MeetingBatchItemResult>[];
    for (final id in ids) {
    ...
    return MeetingBatchOperationResult(items: List.unmodifiable(results));
  }

  Future<MeetingBatchOperationResult> softDelete(
    Iterable<int> recordingIds,
  ) async {
    final ids = _stableIds(recordingIds);
    final recordings = await _recordingsRepository.findByIds(ids);
  ...
  Future<MeetingBatchOperationResult> permanentlyDelete(
    Iterable<int> recordingIds,
  ) async {
    final ids = _stableIds(recordingIds);
    final recordings = await _recordingsRepository.findByIds(ids);
    ...
    for (final id in ids) {
      ...
      if (recording == null) {
        ...
        try {
          final deletion = await _meetingDeletionCoordinator.permanentlyDelete(
            id,
          );
          results.add(
            deletion.completed
  ...
  Future<MeetingBatchOperationResult> retry(Iterable<int> recordingIds) async {
    final ids = _stableIds(recordingIds);
    final recordings = await _recordingsRepository.findByIds(ids);
    final activeIds = ids
        .where(
  ...
  Future<MeetingBatchExportResult> export(
    Iterable<int> recordingIds, {
    ...
    final preliminary = <int, MeetingBatchItemResult>{};
    for (final id in ids) {
      final recording = recordings[id];
      if (recording == null) {
      ...
      entries.add(
        EphemeralArchiveEntry(
          ...
          write: (target) async {
            ...
            await _meetingExportService.exportToFile(
              destination: target,
              title: title,
              segments: segments,
              format: format,
    ...
    if (entries.isEmpty) {
      return MeetingBatchExportResult(
    ...
    final results = ids
        .map((id) {
          ...
          if (entry.succeeded) {
            return MeetingBatchItemResult(
              recordingId: id,
              status: MeetingBatchItemStatus.succeeded,
              reason: 'exported',
  ...
  Future<EphemeralShareReceipt> shareExport(MeetingBatchExportResult result) {
  ...
  String _manifestJson({
    ...
    required Map<int, MeetingBatchItemResult> preliminary,
          ...
          .map((id) {
            ...
            return <String, Object?>{
              ...
              'status': fixed != null
                  ...
                  ? MeetingBatchItemStatus.succeeded.name
                  : entry?.errorCode == 'transcript_unavailable'
                  ? MeetingBatchItemStatus.skipped.name
                  : MeetingBatchItemStatus.failed.name,
              'reason':
                  fixed?.reason ??
                  (entry?.succeeded == true
                      ? 'exported'
...
MeetingBatchItemResult _succeeded(int id, String reason) {
  return MeetingBatchItemResult(
    recordingId: id,
    status: MeetingBatchItemStatus.succeeded,
    reason: reason,
...
MeetingBatchItemResult _skipped(int id, String reason) {
  return MeetingBatchItemResult(
    recordingId: id,
    status: MeetingBatchItemStatus.skipped,
    reason: reason,
  );
}

MeetingBatchItemResult _failed(int id, String reason) {
  return MeetingBatchItemResult(
    recordingId: id,
    status: MeetingBatchItemStatus.failed,
    reason: reason,
  );
}
...
String _recordingTitle(RecordingEntity recording) {
  ...
  return pathTitle.isNotEmpty ? pathTitle : 'meeting-${recording.id}';
}
