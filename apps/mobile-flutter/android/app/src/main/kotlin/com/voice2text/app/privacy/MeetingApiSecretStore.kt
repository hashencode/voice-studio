...
internal data class EncryptedMeetingApiSecret(
    val initializationVector: ByteArray,
    val ciphertext: ByteArray,
)

internal object MeetingApiSecretCipher {
    ...
    fun encrypt(
        ...
        return EncryptedMeetingApiSecret(
    ...
    fun decrypt(
        envelope: EncryptedMeetingApiSecret,
        key: SecretKey,
    ): ByteArray {
        val cipher = Cipher.getInstance(TRANSFORMATION)
        cipher.init(
...
class MeetingApiSecretStore(
    ...
    fun get(providerId: String): String? {
        ...
        return try {
            val plaintext =
                MeetingApiSecretCipher.decrypt(
                    envelope =
                        EncryptedMeetingApiSecret(
            ...
            delete(provider)
            throw MeetingApiSecretUnavailableException()
        }
    }
    ...
    private fun alias(providerId: String): String =
        "voice2text.meeting_ai.$providerId.v1"
    ...
    companion object {
        ...
        private const val PREFERENCES_NAME = "meeting_api_secrets"
        private const val MAX_SECRET_LENGTH = 4096
        private val PROVIDER_ID = Regex("[a-z0-9_-]{1,40}")
    }
}

class MeetingApiSecretUnavailableException :
    IllegalStateException("meeting API secret is unavailable")
