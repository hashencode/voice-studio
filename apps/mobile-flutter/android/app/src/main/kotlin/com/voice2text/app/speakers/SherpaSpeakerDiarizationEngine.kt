...
internal class SherpaSpeakerDiarizationEngine(
    ...
    private fun processSamples(
        ...
        numThreads: Int = PRODUCTION_NUM_THREADS,
        ...
        require(numThreads > 0) { "说话人线程数必须为正数" }

        if (
            configuredNumberOfSpeakers != numberOfSpeakers ||
                configuredNumThreads != numThreads
        ) {
            diarizer?.release()
            diarizer = null
        }
        ...
        val engine = diarizer ?: run {
            ...
            createDiarizer(numberOfSpeakers, numThreads).also {
    ...
    override fun close() {
        ...
        configuredNumThreads = null
        closed = true
    }

    /**
     * Benchmark-only official-parity surface.
     *
     * It passes the complete fixed fixture directly to Sherpa's process call.
     * Production and product-integration paths continue to use bounded windows
     * with the fixed two-thread default.
     */
    fun processFullFixtureForDiagnostic(
        samples: FloatArray,
        numThreads: Int,
    ): List<DiarizationTurn> {
        require(numThreads in FINAL_DIAGNOSTIC_THREAD_MATRIX) {
            "最终诊断线程数必须来自固定矩阵"
        }
    ...
    private companion object {
        ...
        const val PRODUCTION_NUM_THREADS = 2
        val FINAL_DIAGNOSTIC_THREAD_MATRIX = setOf(1, 2, 4)
