...
class AudioRangeVttShareSmokeTest {
    @Test
    fun testRangeVttCanBeReadBySystemReceiver() {
        ...
        val source =
            File(targetContext.filesDir, "audios/device-smoke/range.vtt").apply {
                parentFile?.mkdirs()
                writeText(VTT_CONTENT, Charsets.UTF_8)
            }
        val expectedBytes = source.readBytes()
        val prepared =
            AudioShareCoordinator(targetContext).prepareShare(
        ...
        assertTrue(AudioShareCoordinator(targetContext).discardExport(prepared.path))
