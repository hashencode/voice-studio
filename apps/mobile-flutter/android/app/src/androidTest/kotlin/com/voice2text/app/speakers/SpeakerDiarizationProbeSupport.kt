...
internal object SpeakerDiarizationProbeSupport {
    ...
    const val FINAL_DIAGNOSTIC_CONTRACT_ID =
        "speaker-diarization-final-diagnostic/v1"
    ...
    fun finalDiagnosticArmId(): String =
        androidx.test.platform.app.InstrumentationRegistry.getArguments()
            .getString("speakerDiarizationDiagnosticArm")
            ?: error("speakerDiarizationDiagnosticArm is required")

    fun finalDiagnosticThreads(armId: String): Int = when (armId) {
        "official-3dspeaker-t1" -> 1
        "official-3dspeaker-t2", "official-titanet-t2" -> 2
        "official-3dspeaker-t4" -> 4
        else -> error("unknown final diagnostic arm: $armId")
    }

    fun finalDiagnosticEmbedding(context: Context, armId: String): File =
        File(
            root(context),
            if (armId == "official-titanet-t2") {
                "models/nemo-en-titanet-small.onnx"
            } else {
                "models/3dspeaker-eres2net-base.onnx"
            },
        )

    fun finalDiagnosticRegistry(
        context: Context,
        armId: String,
    ): SpeakerDiarizationModelRegistry {
        val embeddingFile = finalDiagnosticEmbedding(context, armId)
        val embeddingIdentity =
            if (armId == "official-titanet-t2") {
                SpeakerDiarizationModelRegistry.candidateEmbedding.copy(
                    ...
                    sourceVersion = "speaker-recongition-models@2024-12-08",
                    expectedSha256 = TITANET_SHA256,
                    expectedBytes = TITANET_BYTES,
        ...
        return SpeakerDiarizationModelRegistry(
            context = context,
            segmentation =
    ...
    fun finalDiagnosticConfiguration(
        armId: String,
        registry: SpeakerDiarizationModelRegistry,
    ): JSONObject =
        JSONObject()
            .put("mode", "OFFICIAL_PARITY")
            .put("numThreads", finalDiagnosticThreads(armId))
            .put("directFullFixtureProcess", true)
            .put("usesWindowing", false)
            .put("usesExternalEmbedding", false)
            .put("usesReconciliation", false)
            .put("segmentationSha256", registry.segmentation.expectedSha256)
            .put("embeddingSha256", registry.embedding.expectedSha256)
            .put("clusteringAlgorithm", "fast_clustering")
            .put("numSpeakers", 2)
            .put("clusteringThreshold", 0.5)
            .put("minDurationOn", 0.3)
            .put("minDurationOff", 0.5)

    fun readCompleteFixture(file: File): CompletePcmFixture {
        val reader = WavPcmChunkReader(file)
        require(reader.format.totalSamples <= Int.MAX_VALUE) {
            "最终诊断 fixture 超过单次官方 process 的安全上限"
        }
        val samples = FloatArray(reader.format.totalSamples.toInt())
        var consumed = 0
        reader.readChunks(maxSamplesPerChunk = 16_000) { chunkStart, chunk ->
            check(chunkStart == consumed.toLong()) { "最终诊断 PCM chunk 不连续" }
            chunk.copyInto(samples, destinationOffset = consumed)
            consumed += chunk.size
        }
        check(consumed == samples.size) { "最终诊断 fixture 未完整读取" }
        return CompletePcmFixture(
            samples = samples,
            sampleRate = reader.format.sampleRate,
            totalSamples = reader.format.totalSamples,
        )
    }
