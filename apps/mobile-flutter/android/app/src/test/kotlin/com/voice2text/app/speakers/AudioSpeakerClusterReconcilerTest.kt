...
class AudioSpeakerClusterReconcilerTest {
    @Test
    fun localSpeakerIndexSwapKeepsAudioGlobalKeysStable() {
        val reconciler = AudioSpeakerClusterReconciler(similarityThreshold = 0.8)
        ...
        assertEquals("speaker_1", first.assignmentFor(0).audioSpeakerKey)
        assertEquals("speaker_2", first.assignmentFor(1).audioSpeakerKey)
        assertEquals("speaker_2", second.assignmentFor(0).audioSpeakerKey)
        assertEquals("speaker_1", second.assignmentFor(1).audioSpeakerKey)
    }
    ...
    fun oneWindowCannotGreedilyMapTwoClustersToOnePrototype() {
        ...
        assertEquals("speaker_1", assignments.assignmentFor(0).audioSpeakerKey)
        assertEquals(SpeakerAssignmentKind.UNKNOWN, assignments.assignmentFor(1).kind)
        assertNull(assignments.assignmentFor(1).audioSpeakerKey)
    ...
    fun reliableEmbeddingBelowEveryThresholdCreatesNewAnonymousSpeaker() {
        ...
        assertEquals("speaker_2", assignment.audioSpeakerKey)
    ...
    fun missingReliableEmbeddingRemainsUnknown() {
        ...
        assertNull(assignment.audioSpeakerKey)
    ...
    private fun List<AudioSpeakerAssignment>.assignmentFor(
        localSpeakerIndex: Int,
    ): AudioSpeakerAssignment = single { it.localSpeakerIndex == localSpeakerIndex }
}
