...
void main() {
  ...
  testWidgets(
    ...
    (tester) async {
      final harness = await AudioIntegrationHarness.create('offline-flow');
      ...
      final intelligence = AudioIntelligenceRepository(
        database: harness.appDatabase,
      );
      final review = AudioIntelligenceReviewService(repository: intelligence);
      ...
      final controller = AudioReviewController(
        ...
        searchService: AudioSearchService(database: harness.appDatabase),
      ...
      try {
        ...
        if (accessibilityGate) {
          final context = tester.element(find.byType(AudioDetailPage));
          expect(
        ...
        await controller.seekToSegment(controller.audio!.segments[1]);
        expect(controller.currentSegmentIndex, 1);
        await controller.saveSegment(
          segmentId: segmentIds[1],
          text: 'Decision: ship safely',
        );
        expect(controller.audio!.segments[1].text, 'Decision: ship safely');
        ...
        final exporter = AudioExportService(
          recordingsRepository: recordings,
        ...
        for (final format in AudioExportFormat.values) {
          final receipt = await exporter.export(
            recordingId: recordingId,
            title: controller.audio!.title,
            segments: controller.audio!.segments,
            format: format,
          );
        ...
        final rangedVtt = await exporter.export(
          ...
          selection: AudioExportSelection.range(
            AudioTimeRange(startMs: 2000, endMs: 5000, durationMs: 5000),
          ),
        );
  ...
  testWidgets(
    ...
    (tester) async {
      final harness = await AudioIntegrationHarness.create('large-flow');
      addTearDown(harness.dispose);
      final db = harness.database;
      final recordingId = await db.insert('recordings', <String, Object?>{
        'file_path': harness.audioFile.path,
      ...
      addTearDown(controller.dispose);
      await controller.load();
      expect(controller.audio!.segments, hasLength(3000));
      ...
      await controller.seekToSegment(controller.audio!.segments.last);
      await tester.pump();
      ...
      final finalSegment = controller.audio!.segments.last;
      expect(
        await controller.updateReviewState(
...
class _IntegrationPlaybackBackend implements AudioPlaybackBackend {
