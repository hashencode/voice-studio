...
class MeetingIntegrationHarness {
  MeetingIntegrationHarness._({
  ...
  static Future<MeetingIntegrationHarness> create(String name) async {
    final root = await Directory.systemTemp.createTemp('voice2text-$name-');
    final audio = File(p.join(root.path, 'meeting.wav'));
    ...
    return MeetingIntegrationHarness._(
      database: database,
