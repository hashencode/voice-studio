...
export interface MeetingAiInputSegment {
  id: number;
  startMs: number;
  endMs: number;
  text: string;
...
const wireOutputSchema = z
  .object({
    schema_version: z.literal("meeting_intelligence_output/v1"),
    suggested_title: z.string().trim().min(1).max(512).nullable(),
    meeting_type: z.string().trim().min(1).max(128).nullable(),
    items: z
      .array(
        z
          .object({
...
export function decodeMeetingAiOutput(
  source: string,
  inputSegments: readonly MeetingAiInputSegment[],
): MeetingAiOutput {
