...
import type { MeetingAiInputSegment, MeetingAiOutput } from "./provider_output";
import { decodeMeetingAiOutput } from "./provider_output";
import {
  AiProviderFailure,
  BoundedOpenAiClient,
...
export interface MeetingAiProviderRequest {
  meetingTitle: string;
  templateId: string;
...
export class OpenAiCompatibleProvider {
  ...
  async generate(request: MeetingAiProviderRequest): Promise<MeetingAiOutput> {
        ...
        messages: [
          {
            role: "system",
            content:
              "Generate only meeting_intelligence_output/v1 JSON from the supplied transcript. " +
    ...
    return decodeMeetingAiOutput(choice.message.content, request.segments);
  }
...
const meetingAiJsonSchema = {
  type: "object",
  additionalProperties: false,
  required: ["schema_version", "suggested_title", "meeting_type", "items"],
  properties: {
    schema_version: { type: "string", const: "meeting_intelligence_output/v1" },
