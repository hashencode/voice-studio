...
import { AudioWorkspaceService } from "./audio_workspace_service";
...
export class AudioExportService {
  ...
  async exportAudio(
    ...
    const snapshot = this.workspace.openAudio(audioId);
    if (!snapshot?.summary.generationId) {
      throw new Error(
        "audio has no complete authoritative transcript to export",
      );
    }
...
function render(
  snapshot: AudioWorkspaceSnapshot,
  format: AudioExportFormat,
): string {
  switch (format) {
    case "txt":
...
function timed(snapshot: AudioWorkspaceSnapshot, webVtt: boolean): string {
...
function speaker(segment: AudioSegment): string {
...
function mimeType(format: AudioExportFormat): string {
  return {
    txt: "text/plain",
    md: "text/markdown",
    vtt: "text/vtt",
