...
export function MeetingAiFeature({
  meetingId,
  generationId,
  ...
  const [snapshot, setSnapshot] = React.useState<MeetingAiSnapshot | null>(
    null,
  );
  const [pending, setPending] = React.useState(false);
  const [error, setError] = React.useState<string | null>(null);
  const [preview, setPreview] = React.useState<MeetingAiConsentPreview | null>(
    null,
  );
  ...
  const accept = React.useCallback(
    (next: MeetingAiSnapshot) => {
      if (next.meetingId !== meetingId || next.generationId !== generationId)
        return;
      setSnapshot((current) =>
        !current || isNewerSnapshot(current, next) ? next : current,
      );
    },
    [generationId, meetingId],
  );

  React.useEffect(() => {
    let active = true;
    const unsubscribe = api.onMeetingAiSnapshot((next) => {
      if (active) accept(next);
    });
    void api
      .getMeetingAiSnapshot({ meetingId })
      .then((next) => {
  ...
  const prepare = async (kind: RequestTarget["kind"]) => {
    ...
    setError(null);
    try {
      const next = await api.prepareMeetingAi({
        meetingId,
        generationId,
        templateId: "default",
      });
    ...
    try {
      const next =
        requestTarget.kind === "retry"
          ? await api.retryMeetingAi({
              jobId: requestTarget.jobId,
              expectedAttempt: requestTarget.expectedAttempt,
              idempotencyKey: requestIdentity("meeting-ai-retry"),
              consent,
            })
          : await api.generateMeetingAi({
              meetingId,
              generationId,
              templateId: "default",
              idempotencyKey: requestIdentity("meeting-ai-generate"),
              consent,
            });
      accept(next);
    } catch (cause) {
  ...
  return (
    <section
      aria-labelledby="meeting-ai-title"
      className="rounded-xl border bg-card p-4"
    >
      <div className="flex items-start gap-3">
        <BrainCircuit className="mt-0.5 size-5" aria-hidden="true" />
        <div className="min-w-0 flex-1">
          <h2 id="meeting-ai-title" className="font-semibold">
            会议智能草稿
          </h2>
      ...
      {snapshot?.state === "completed" && snapshot.note ? (
        <MeetingAiNote snapshot={snapshot} />
      ) : null}
...
function ConsentDialog({
  ...
  preview: MeetingAiConsentPreview | null;
  checked: boolean;
  pending: boolean;
          ...
          {preview ? (
            <>
              <DialogPrimitive.Description className="mt-2 text-sm text-muted-foreground">
                将会议标题“{preview.meetingTitle}”以及本次会议的{" "}
...
function MeetingAiNote({ snapshot }: { snapshot: MeetingAiSnapshot }) {
