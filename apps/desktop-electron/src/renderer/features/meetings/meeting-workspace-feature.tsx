...
export function MeetingWorkspaceFeature({
  ...
  const [workspace, setWorkspace] =
    React.useState<MeetingWorkspaceSnapshot | null>(null);
  ...
  const loadMeetings = React.useCallback(
    async (query = "") => {
      ...
      setError(null);
      try {
        const next = await api.listMeetings(query);
        setMeetings(next);
        setStatus(
  ...
  const openMeeting = async (meetingId: number) => {
    ...
    setError(null);
    try {
      const next = await api.openMeeting(meetingId);
  ...
  if (meetings === null && !error) return <MeetingLoading />;
  if (!workspace) {
    return (
      <section
      ...
      >
        ...
        <form
          ...
          onSubmit={(event) => {
            ...
            void loadMeetings(libraryQuery);
          }}
          role="search"
        >
          <Input
        ...
        {error ? (
          <RecoveryError
            message={error}
            pending={pending}
            onRetry={() => void loadMeetings(libraryQuery)}
          />
        ) : meetings?.length === 0 ? (
          ...
          >
            {meetings?.map((meeting) => (
              <li key={meeting.meetingId}>
                <button
                  type="button"
                  aria-label={`打开 ${meeting.displayName}`}
                  ...
                  disabled={pending}
                  onClick={() => void openMeeting(meeting.meetingId)}
                >
                  <span className="block truncate font-medium">
                    {meeting.displayName}
                  </span>
                  <span className="mt-2 block text-sm text-muted-foreground">
                    {meeting.segmentCount} 个片段 ·{" "}
                    {processingLabel(meeting.processingState)}
                  </span>
                </button>
              </li>
            ))}
  ...
  return (
    <WorkspaceView
      ...
      onBack={() => {
        const meetingId = workspace.summary.meetingId;
        setWorkspace(null);
        setError(null);
        setStatus("已返回会议资料库");
...
function WorkspaceView({
  ...
  api: Voice2TextDesktopApi;
  workspace: MeetingWorkspaceSnapshot;
  pending: boolean;
  error: string | null;
  status: string;
  ...
  setWorkspace: (value: MeetingWorkspaceSnapshot) => void;
  onBack: () => void;
}) {
  const [query, setQuery] = React.useState("");
  const [playback, setPlayback] =
    React.useState<MeetingPlaybackSnapshot | null>(null);
  ...
  const requestPlaybackClose = React.useCallback(() => {
    ...
    const request = api
      .controlMeetingPlayback(workspace.summary.meetingId, { action: "close" })
      .then(() => undefined);
    ...
    return request;
  }, [api, workspace.summary.meetingId]);
  ...
  const mutate = async (
    action: () => Promise<MeetingWorkspaceSnapshot>,
    success: string,
  ) => {
    if (operationPendingRef.current) return;
    operationPendingRef.current = true;
  ...
  const playbackAction = async (
    command: Parameters<Voice2TextDesktopApi["controlMeetingPlayback"]>[1],
  ) => {
    if (operationPendingRef.current) return;
    operationPendingRef.current = true;
    setPending(true);
    ...
    try {
      ...
      if (!current?.initialized && command.action !== "open") {
        current = await api.controlMeetingPlayback(
          workspace.summary.meetingId,
          { action: "open" },
        );
      }
      const next =
        command.action === "open"
          ? current!
          : await api.controlMeetingPlayback(
              workspace.summary.meetingId,
              command,
            );
      setPlayback(next);
  ...
  const exportMeeting = async (format: MeetingExportFormat) => {
    ...
    setError(null);
    try {
      const result = await api.exportMeeting(
        workspace.summary.meetingId,
        format,
      );
    ...
    >
      ...
      {error ? (
        <RecoveryError
          ...
          onRetry={() =>
            void mutate(async () => {
              const next = await api.openMeeting(workspace.summary.meetingId);
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_18rem]">
        <div className="space-y-4">
          <form
            ...
            onSubmit={(event) => {
              ...
              setPending(true);
              void api
                .searchTranscript(workspace.summary.meetingId, query)
                .then((results) => {
        ...
        <aside aria-label="会议操作" className="space-y-4">
          ...
          <ExportPanel
            pending={pending}
            onExport={(format) => void exportMeeting(format)}
          />
        </aside>
      </div>
      <LiveStatus status={status} />
...
function VirtualTranscript({
  ...
  onEdit: (segment: MeetingSegment, text: string) => void;
  onAssign: (
...
function SegmentRow({
  ...
  segment: MeetingSegment;
  speakers: MeetingWorkspaceSnapshot["speakers"];
  pending: boolean;
  top: number;
  total: number;
  onEdit: (segment: MeetingSegment, text: string) => void;
...
function PlaybackPanel({
  ...
  playback: MeetingPlaybackSnapshot | null;
  durationMs: number;
  pending: boolean;
  onAction: (
    command: Parameters<Voice2TextDesktopApi["controlMeetingPlayback"]>[1],
...
function speakerLabel(segment: MeetingSegment): string {
  if (segment.speakerState === "overlap") return "多人重叠";
  if (segment.speakerState === "unknown") return "未知说话人";
  return segment.speakerName ?? "匿名说话人";
}

function processingLabel(state: MeetingSummary["processingState"]): string {
  return {
    queued: "等待处理",
    running: "处理中",
    canceling: "正在取消",
...
function generationLabel(kind: MeetingSummary["generationKind"]): string {
  if (kind === "formal") return "正式转写";
  if (kind === "live-draft") return "实时草稿";
  return "转写尚未就绪";
}

function playbackStatus(snapshot: MeetingPlaybackSnapshot): string {
