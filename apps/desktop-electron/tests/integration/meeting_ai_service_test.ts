...
import { MeetingAiService } from "../../src/main/domain/meeting-intelligence/meeting_ai_service";
import { AiProviderFailure } from "../../src/main/domain/meeting-intelligence/provider_security";
...
describe("U10 meeting AI vertical slice", () => {
  ...
  it("prepares settings and consent scope without secret access or network", () => {
    ...
    const service = new MeetingAiService(
  ...
  it("invalidates consent when the exact provider meeting title changes", async () => {
    database = openElectronDatabase(":memory:");
    seedMeeting(database);
    const secrets = secretStore();
    const generate = vi.fn(async () => output());
    ...
    database
      .prepare("UPDATE meetings SET display_name = ? WHERE id = 1")
      .run("Renamed meeting");
...
function consent(preview: ReturnType<MeetingAiService["prepare"]>) {
  return {
    version: 1 as const,
...
function seedMeeting(database: DatabaseSync): void {
  database.exec(`
    ...
    ) VALUES (1, 'meeting-1', 'source-1', 'Meeting', '/private/tmp/media.wav', 1000, 1, 1);
    ...
    INSERT INTO meeting_generations (
      id, meeting_id, publication_id, kind, attempt, partial_success, created_at_ms
    ) VALUES (1, 1, 1, 'formal', 1, 0, 1);
    UPDATE meetings SET active_generation_id = 1 WHERE id = 1;
    INSERT INTO transcript_segments (
      id, meeting_id, generation_id, stable_key, sequence_id, machine_text, text,
