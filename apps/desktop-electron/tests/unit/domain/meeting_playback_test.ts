...
} from "../../../src/main/features/playback/meeting_playback_service";
...
describe("Main-owned meeting playback", () => {
  it("resolves media privately and accepts only bounded opaque actions", async () => {
    ...
    const service = new MeetingPlaybackService(
      {
        resolvePlayback: () => ({
          mediaPath: "/private/media/meeting.wav",
          durationMs: 10_000,
        }),
      },
      port,
    ...
    expect(await service.command({ meetingId: 7, action: "open" })).toEqual(
    ...
    await service.command({ meetingId: 7, action: "seek", positionMs: 99_000 });
    await service.command({ meetingId: 7, action: "speed", speed: 2 });
    await service.command({ meetingId: 7, action: "play" });
    expect(calls).toEqual([
      ["open", "/private/media/meeting.wav"],
      ["seek", 10_000],
      ["speed", 2],
      ["play"],
    ]);
    ...
    await expect(
      service.command({ meetingId: 7, action: "speed", speed: 2.01 }),
    ).rejects.toThrow(/speed/i);
    await expect(
      service.command({ meetingId: 8, action: "play" }),
  ...
  it("destroys Main-owned playback state when media resolution or opening fails", async () => {
    ...
    await expect(
      service.command({ meetingId: 1, action: "open" }),
    ).rejects.toThrow(/decode/i);
    expect(port.close).toHaveBeenCalledOnce();
    expect(service.snapshot()).toEqual(
      expect.objectContaining({ meetingId: null, initialized: false }),
    );
  ...
  it("destroys the hidden BrowserWindow after document-load and decode failures", async () => {
    ...
    roots.push(root);
    const media = join(root, "meeting.wav");
