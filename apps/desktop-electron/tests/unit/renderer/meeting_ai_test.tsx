...
import { MeetingAiFeature } from "../../../src/renderer/features/meeting-ai/meeting-ai-feature";
import type { Voice2TextDesktopApi } from "../../../src/shared/contracts";
import type { MeetingAiSnapshot } from "../../../src/shared/contracts";
...
const completed: MeetingAiSnapshot = {
  revision: 4,
  jobId: 41,
  meetingId: 4,
  generationId: 9,
...
function api(overrides: Record<string, unknown> = {}) {
  return {
    ...
    prepareMeetingAi: vi.fn(async () => preview),
    generateMeetingAi: vi.fn(async () => completed),
    retryMeetingAi: vi.fn(async () => ({
      ...completed,
      revision: 6,
      attempt: 2,
    })),
...
describe("per-generation meeting AI consent", () => {
  it("cancels with Escape, restores focus, and sends no transcript", async () => {
    const desktop = api();
    const user = userEvent.setup();
    render(<MeetingAiFeature api={desktop} meetingId={4} generationId={9} />);
    ...
    expect(desktop.prepareMeetingAi).toHaveBeenCalledWith({
      meetingId: 4,
      generationId: 9,
      templateId: "default",
    });
  ...
  it("passes the exact prepared scope and requires fresh consent each time", async () => {
    ...
    await waitFor(() =>
      expect(desktop.generateMeetingAi).toHaveBeenCalledWith(
    ...
    expect(desktop.prepareMeetingAi).toHaveBeenCalledTimes(2);
    expect(desktop.generateMeetingAi).toHaveBeenCalledTimes(1);
  });
  ...
  it("shows provider failure without automatic fallback or retry", async () => {
    ...
    const desktop = api({
      generateMeetingAi: vi.fn(async () => Promise.reject(failure)),
    });
    const user = userEvent.setup();
    render(<MeetingAiFeature api={desktop} meetingId={4} generationId={9} />);
  ...
  it("retries an interrupted attempt once with fresh exact-scope consent", async () => {
    const interrupted: MeetingAiSnapshot = {
      ...completed,
      revision: 5,
      attempt: 1,
      state: "interrupted",
    ...
    const desktop = api({
      getMeetingAiSnapshot: vi.fn(async () => interrupted),
    });
    const user = userEvent.setup();
    render(<MeetingAiFeature api={desktop} meetingId={4} generationId={9} />);
    ...
    await waitFor(() =>
      expect(desktop.retryMeetingAi).toHaveBeenCalledWith(
        expect.objectContaining({
          jobId: 41,
          expectedAttempt: 1,
    ...
    expect(desktop.retryMeetingAi).toHaveBeenCalledTimes(1);
    expect(desktop.generateMeetingAi).not.toHaveBeenCalled();
  });
  ...
  it("offers a fresh generation when a failed attempt's prior scope can no longer be retried", async () => {
    ...
    await waitFor(() =>
      expect(desktop.generateMeetingAi).toHaveBeenCalledWith(
        expect.objectContaining({ meetingId: 4, generationId: 9 }),
      ),
    );
    expect(desktop.retryMeetingAi).not.toHaveBeenCalled();
  });
  ...
  it("keeps the retry job and attempt captured before the consent dialog opened", async () => {
    ...
    let listener: ((snapshot: MeetingAiSnapshot) => void) | undefined;
    const desktop = api({
      getMeetingAiSnapshot: vi.fn(async () => interrupted),
      onMeetingAiSnapshot: vi.fn((next) => {
        listener = next;
        return () => undefined;
      }),
    });
  ...
  it("ignores stale full-snapshot events by job, attempt, and revision fences", async () => {
    let listener: ((snapshot: MeetingAiSnapshot) => void) | undefined;
    const desktop = api({
      getMeetingAiSnapshot: vi.fn(async () => completed),
      onMeetingAiSnapshot: vi.fn((next) => {
        listener = next;
        return () => undefined;
      }),
