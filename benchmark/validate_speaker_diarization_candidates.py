...
def validate_candidates(
    ...
    final_diagnostic = matrix.get("finalDiagnostic") or {}
    _require(
        final_diagnostic.get("terminalDisposition")
        == "MOBILE_DIARIZATION_CLOSED_NO_ADMISSIBLE_CANDIDATE",
        "final mobile diagnostic must be terminally closed",
    )
    _require(
        final_diagnostic.get("testedArmIds")
        == [
        ...
        "final mobile diagnostic arm set changed",
    )
    _require(
        final_diagnostic.get("nextCandidate") is None,
        "final mobile diagnostic cannot name a next candidate",
    )
    _require(
        _is_sha256(final_diagnostic.get("summarySha256")),
        "final mobile diagnostic summary hash missing",
    )
    return {
