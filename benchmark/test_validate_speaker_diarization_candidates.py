...
class SpeakerDiarizationCandidatesTest(unittest.TestCase):
    ...
    def test_final_diagnostic_cannot_reopen_candidate_loop(self):
        matrix = copy.deepcopy(self.matrix)
        matrix["finalDiagnostic"]["nextCandidate"] = "third-candidate"

        with self.assertRaisesRegex(CandidateMatrixError, "next candidate"):
            validate_candidates(matrix, self.contract)
