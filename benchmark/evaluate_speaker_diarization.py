...
def _frames_for_region(
    ...
    *,
    ...
    _require(
        ...
        and float(region[1]) > float(region[0]),
        "diagnostic region invalid",
    )
    first = max(0, math.floor(float(region[0]) / step))
    last = min(len(frames), math.ceil(float(region[1]) / step))
    _require(last > first, "diagnostic region is outside the fixture")
...
def evaluate_final_diagnostic(
    ...
    *,
    rttm_text: str,
    overlap_regions: list[list[float]],
    silence_regions: list[list[float]],
) -> dict[str, Any]:
    """Evaluate one direct full-fixture official-parity diagnostic arm."""
    validate_final_diagnostic_evidence(evidence, arm, contract)
    fixture = evidence["fixture"]
    sample_rate = fixture["sampleRate"]
    total_samples = fixture["totalSamples"]
    ...
    overlap_represented = bool(overlap_regions) and all(
        any(len(frame) >= 2 for frame in _frames_for_region(hypothesis_frames, region))
        for region in overlap_regions
    )
    silence_honest = bool(silence_regions) and all(
        all(not frame for frame in _frames_for_region(hypothesis_frames, region))
        for region in silence_regions
    )
    thresholds = contract["thresholds"]
    failed_gates: list[str] = []
    if coverage < float(thresholds["minimumAnnotatedSpeechCoverage"]):
        failed_gates.append("COVERAGE")
    if der > float(thresholds["maximumDer"]):
        failed_gates.append("DER")
    if thresholds["requiresOverlapRepresentation"] and not overlap_represented:
        failed_gates.append("OVERLAP")
    if thresholds["requiresNoSpeakerInPreregisteredSilence"] and not silence_honest:
        failed_gates.append("SILENCE")
    if evidence["timings"]["rtf"] > float(thresholds["maximumRtf"]):
        failed_gates.append("RTF")
    return {
        "armId": arm["id"],
        ...
        "der": der,
        "overlapRepresented": overlap_represented,
        "noSpeakerInPreregisteredSilence": silence_honest,
        "rtf": evidence["timings"]["rtf"],
        "elapsedMs": evidence["timings"]["elapsedMs"],
        "incrementalPeakPssKiB": max(
            0,
            evidence["resources"]["peakPssKiB"]
            - evidence["resources"]["baselinePssKiB"],
        ),
        "maximumThermalStatus": evidence["device"]["maximumThermalStatusName"],
        "completeInputConsumption": (
            fixture["consumedSamples"] == fixture["totalSamples"]
        ),
        "transcriptSnapshotHashMatch": (
            evidence["transcriptSnapshot"]["beforeSha256"]
            == evidence["transcriptSnapshot"]["afterSha256"]
        ),
    }
...
def main() -> int:
    ...
    parser.add_argument(
        "--phase",
        choices=("validate", "screen", "final", "diagnostic"),
        default="validate",
    )
    ...
    try:
        ...
        if args.phase == "diagnostic":
            validate_final_diagnostic_contract(contract, require_terminal=False)
            _require(args.generated is not None, "generated fixture manifest is required")
            _require(args.diagnostic_evidence is not None, "diagnostic evidence is required")
            _require(args.rttm is not None, "RTTM is required")
            _require(bool(args.arm_id), "diagnostic arm id is required")
            generated = json.loads(args.generated.read_text(encoding="utf-8"))
            rttm_bytes = args.rttm.read_bytes()
            _require(
                hashlib.sha256(rttm_bytes).hexdigest()
                == (contract.get("fixture") or {}).get("rttmSha256"),
                "RTTM hash does not match final diagnostic contract",
            )
            functional = generated.get("functional") or {}
            _require(
                functional.get("sha256")
                == (contract.get("fixture") or {}).get("wavSha256"),
                "generated fixture does not match final diagnostic contract",
            )
            arm = next(
                (
                    item
                    for item in contract.get("parityArms") or []
                    if item.get("id") == args.arm_id
                ),
                None,
            )
            _require(isinstance(arm, dict), "diagnostic arm is not registered")
            diagnostic_bytes = args.diagnostic_evidence.read_bytes()
            result = evaluate_final_diagnostic(
                json.loads(diagnostic_bytes),
                arm,
                contract,
                rttm_text=rttm_bytes.decode("utf-8"),
                overlap_regions=functional.get("overlapRegions") or [],
                silence_regions=functional.get("silenceRegions") or [],
            )
            result["evidenceSha256"] = hashlib.sha256(diagnostic_bytes).hexdigest()
            if args.report:
            ...
            if args.phase != "validate":
                ...
                _require(
                    all(common_paths),
                    "generated, five-minute evidence, and RTTM are required",
                )
                if args.phase == "final":
    ...
    if "armId" in result:
        print(
            f"{result['status']}: arm={result['armId']} "
            f"failedGates={result['failedGates']} rtf={result['rtf']:.4f}"
        )
        return 0
