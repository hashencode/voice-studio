...
if len(devices) != 1:
    raise SystemExit("Pass one Android device id: ./tool/run_meeting_flow_smoke.sh <device-id>")
print(devices[0]["id"])
')"
fi
...
echo "[1/4] Offline meeting review flow on $DEVICE_ID"
flutter test integration_test/meeting_offline_flow_test.dart \
  -d "$DEVICE_ID" \
  --plain-name \
  "timestamped meeting can play, edit, search, export and review evidence"

echo "[2/4] Large meeting lazy timeline flow on $DEVICE_ID"
...
flutter test integration_test/meeting_recovery_flow_test.dart -d "$DEVICE_ID"
...
adb -s "$DEVICE_ID" shell am instrument -w -r \
  -e class \
  com.voice2text.app.transcription.PunctuationModelSmokeTest,com.voice2text.app.sharing.MeetingRangeVttShareSmokeTest \
  "$TEST_PACKAGE_NAME/androidx.test.runner.AndroidJUnitRunner"

restore_installed_app
trap - EXIT INT TERM
echo "Meeting flow smoke finished."
