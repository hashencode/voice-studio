...
echo "[6/20] Paired-PC provider protocol contract"
...
echo "[7/20] Companion Audio transfer protocol contract"
...
echo "[8/20] S3 productization truth contract"
python3 -m unittest tool/test_validate_s3_productization_scope.py
python3 tool/validate_s3_productization_scope.py

echo "[9/20] Mobile Flutter layout contract"
python3 tool/update_mobile_flutter_layout.py --check
...
echo "[12/20] Desktop foundation contract"
./tool/check_desktop_foundation.sh

echo "[13/20] Desktop benchmark contract"
./tool/check_desktop_benchmark.sh

echo "[14/20] Shared Dart workspace contract"
...
echo "[17/20] Timestamp fixture contract"
...
echo "[18/20] ASR model-admission contract"
python3 -m unittest \
