...
DEFAULT_SCHEMA = ROOT / "docs/contracts/meeting-intelligence-provider-v1.schema.json"
PROTOCOL_SCHEMA = "meeting_intelligence_provider/v1"
OUTPUT_SCHEMA = "meeting_intelligence_output/v1"
INPUT_SCHEMA = "meeting_intelligence_input/v1"
REQUIRED_CAPABILITY = "meeting_intelligence.generate.v1"
...
def _validate_result(message: dict[str, Any]) -> None:
    ...
    _require(set(output) <= {"schema_version", "meeting_type", "suggested_title", "items"}, "output has unknown fields")
...
def _validate_schema_document(schema: dict[str, Any]) -> None:
    ...
    output = schema.get("$defs", {}).get("meetingOutput", {})
    _require(
...
def sample_messages(now_ms: int = 1_784_990_000_000) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    input_payload = {
        ...
        "input_end_ms": 2_000,
        "segments": [
            {"segment_id": 1, "start_ms": 0, "end_ms": 1_200, "text": "示例会议内容"}
        ],
    }
    request = {
        "schemaVersion": PROTOCOL_SCHEMA,
