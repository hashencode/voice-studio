...
def validate_scope_contract(
    ...
    *,
    ...
    _require(asr005.get("status") == "BLOCKED", "ASR-005 must remain BLOCKED")
    _require(
        asr005.get("executionDisposition") == "SKIPPED_PENDING_USER_TEST",
        "ASR-005 execution must remain skipped pending user testing",
    )
    s2_manifest = json.loads(
        (root / "docs/product/s2-mobile-core-scope.json").read_text(encoding="utf-8")
    )
    _require(
        s2_manifest.get("timestampGate", {}).get("executionDisposition")
        == "SKIPPED_PENDING_USER_TEST",
        "S2 authority no longer reports ASR-005 skipped",
    )
    ...
    _require("ASR-005-TIMESTAMP-INDEPENDENT" in release.get("blockers", []), "ASR-005 blocker missing")
